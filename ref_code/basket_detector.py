...
class BasketDetector:
    ...
    def __init__(self, 
        ...
        Args:
            model_path: YOLO模型文件路径 (.pt文件)，默认使用 weights/Basketball_gym.pt
            class_num: 要检测的单一类别ID，0=篮球, 1=篮筐，默认 1
            best_detection: 是否只返回最佳检测结果，True=只返回最高置信度结果，False=返回所有结果，默认 True
            confidence_threshold: 检测置信度阈值，默认 0.1
            imgsz: 模型输入图像尺寸，默认 416 (较小尺寸减少计算量)
            half: 是否使用半精度推理，默认 False (减少计算量)
            retina_masks: 是否使用视网膜掩码，默认 False (减少计算量)
...
def main():
    ...
    while True:
        ...
        # 简化绘制 - 只绘制最佳检测（如果有）
        if detections:
            detection = detections[0]  # 由于best_detection=True，只有一个检测
            x1, y1, w, h = detection['bbox']
            
            # 简化绘制 - 只画框，不画文字，使用红色表示篮筐
